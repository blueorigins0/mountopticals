...
export default function AdminPromoBanners() {
  ...
  const [form, setForm] = useState({ title: "", offer_text: "", image_url: "", link: "/products", sort_order: "0", is_active: true });
  ...
  const openForm = (b?: PromoBanner) => {
    if (b) {
      setEditing(b);
      setForm({ title: b.title, offer_text: b.offer_text || "", image_url: b.image_url, link: b.link || "/products", sort_order: String(b.sort_order || 0), is_active: b.is_active ?? true });
    } else {
      setEditing(null);
      setForm({ title: "", offer_text: "", image_url: "", link: "/products", sort_order: "0", is_active: true });
    }
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    const data = { title: form.title, offer_text: form.offer_text || null, image_url: form.image_url, link: form.link || "/products", sort_order: parseInt(form.sort_order) || 0, is_active: form.is_active };
    if (editing) {
