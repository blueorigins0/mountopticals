  ...
  return (
    <Html center>
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-foreground text-sm">Loading 3D Model...</p>
      </div>
    </Html>
  );
}
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-b from-muted to-background rounded-lg">
      <Canvas camera={{ position: [0, 0, 3], fov: 45 }} style={{ width: "100%", height: "100%" }}>
        ...
        <OrbitControls enablePan={false} enableZoom={true} minDistance={1} maxDistance={8} autoRotate autoRotateSpeed={2} />
      </Canvas>

      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-background/70 backdrop-blur-sm rounded-full px-4 py-1.5 border border-border">
        <p className="text-muted-foreground text-xs">Drag to rotate • Pinch to zoom</p>
      </div>
    </div>
  );
}
