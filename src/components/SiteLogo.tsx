...
export function SiteLogo({ className = "h-8 w-auto", fallbackText = "V", textClassName, showText = false }: SiteLogoProps) {
  const logoUrl = useSiteLogo();

  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      {logoUrl ? (
        <img src={logoUrl} alt="Logo" className={`${className} object-contain max-h-10 sm:max-h-12`} />
      ) : (
